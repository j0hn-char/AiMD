...
def get_relevant_chunks(ai_diagnosis, papers: list[str]):
    # ---- Chunking όλων των papers μαζί, κρατώντας track ποιο chunk ανήκει σε ποιο paper ----
    all_chunks = []
    paper_indices = []  # για κάθε chunk, σε ποιο paper ανήκει

    for paper_idx, paper in enumerate(papers):
        chunks = split_into_chunks(paper)
        all_chunks.extend(chunks)
        paper_indices.extend([paper_idx] * len(chunks))

    # ---- Batch encoding όλων των chunks μαζί (γρηγορότερο από ένα-ένα) ----
    ...
    # ---- Cosine similarity μαζικά ----
    scores = util.cos_sim(diagnosis_embedding, chunk_embeddings)[0].numpy()

    # ---- Φιλτράρισμα βάσει threshold ----
    # Αποτέλεσμα: dict με key=paper_idx, value=list of relevant chunks
    results = {i: [] for i in range(len(papers))}
    for i, (chunk, score) in enumerate(zip(all_chunks, scores)):
        if score >= THRESHOLD:
            results[paper_indices[i]].append(chunk)
...
if __name__ == "__main__":
    ...
    for paper_idx, chunks in relevant_chunks.items():
        print(f"\n=== Paper {paper_idx + 1}: {len(chunks)} σχετικά αποσπάσματα ===\n")
        for chunk in chunks:
            print(chunk)
            print("---")