...
def convert_inline(text):
    ...
    text = text.replace('β0', 'β0')
    text = text.replace('α+', 'α+')
    text = text.replace('α0', 'α0')
    # Αφαίρεσε markdown links [text](#anchor) → κράτα μόνο το text
...
def markdown_to_flowables(md_text):
    ...
    toc_bullet_style = ParagraphStyle('TocBullet', parent=styles['Normal'],
                                      fontSize=10, leading=14, spaceAfter=3,
                                      leftIndent=36)  # εσοχή για υπο-entries TOC
    ...
    for line in md_text.split('\n'):
        ...
        if stripped.startswith('### '):
            ...
            continue
        # Bullet points — διαφορετική εσοχή αν είναι sub-bullet (•)
        m = re.match(r'^[-*]\s+(.*)', stripped)
        if m:
            indent = len(line) - len(line.lstrip())
            if indent >= 2:  # sub-bullet (π.χ. TOC υπο-entries)
                bs = ParagraphStyle('SubBul', parent=toc_bullet_style, leftIndent=36 + indent)
            else:
